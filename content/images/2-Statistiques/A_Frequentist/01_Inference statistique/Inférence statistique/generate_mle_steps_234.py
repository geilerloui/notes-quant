"""
Génère mle_steps_234.png : les TROIS derniers plots du MLE en grille 1x3
(le plot 1 est désormais isolé dans mle_step1_densities.png).

- Plot ② : Vraisemblance L(mu, sigma_fixé) en fonction de mu — sommet à mu_MLE.
- Plot ③ : Vraisemblance L(mu_fixé, sigma) en fonction de sigma — sommet à sigma_MLE.
- Plot ④ : Log-vraisemblance log L en fonction de mu — sommet au même endroit
           (avec inset pour log L en fonction de sigma).

À placer après §V de la note 04_Estimations.

Output : mle_steps_234.png dans le même dossier que ce script.
"""
...
# ---------------------------------------------------------------------
# Dataset synthétique (mêmes paramètres que mle_step1_densities pour cohérence)
# ---------------------------------------------------------------------
np.random.seed(42)
...
# Setup figure 1x3
# ---------------------------------------------------------------------
fig, (ax2, ax3, ax4) = plt.subplots(1, 3, figsize=(16, 5))

color_lik = '#0F6E56'
color_loglik = '#D85A30'

# ---------------------------------------------------------------------
# PLOT ② — Vraisemblance L en fonction de mu (sigma fixé)
# ---------------------------------------------------------------------
ax = ax2
...
ax.set_title(fr'② Vraisemblance en fonction de $\mu$' + '\n' + fr'($\sigma = {sigma_mle:.2f}$ fixé)',
             fontsize=11, pad=8)
...
# ---------------------------------------------------------------------
# PLOT ③ — Vraisemblance L en fonction de sigma (mu fixé)
# ---------------------------------------------------------------------
ax = ax3
...
ax.set_title(fr'③ Vraisemblance en fonction de $\sigma$' + '\n' + fr'($\mu = {mu_mle:.2f}$ fixé)',
             fontsize=11, pad=8)
...
# ---------------------------------------------------------------------
# PLOT ④ — Log-vraisemblance (en mu, avec inset pour sigma)
# ---------------------------------------------------------------------
ax = ax4
...
ax.plot(mu_grid, logL_mu, color=color_loglik, linewidth=2.2)
...
ax.set_title(r'④ Log-vraisemblance en fonction de $\mu$' + '\n' + r'(même sommet, calculs plus stables)',
             fontsize=11, pad=8)
ax.set_xlabel(r'$\mu$', fontsize=11)
...
ax_inset = ax.inset_axes([0.58, 0.10, 0.38, 0.35])
...
ax_inset.set_title(r'$\log L$ en fonction de $\sigma$', fontsize=8)
...
# ---------------------------------------------------------------------
fig.suptitle(r'Étapes 2 à 4 du MLE — maximisation de la vraisemblance',
             fontsize=13, y=1.02)

fig.text(0.5, -0.03,
         r'Le sommet de la vraisemblance ② et de la log-vraisemblance ④ '
         r'sont au même $\mu$. Idem pour ③.' '\n'
         r'En pratique on travaille avec $\log L$ (plot ④) : sommet identique, '
